...
def distillation(rank, world_size, args):
    ...
    with trange(args.total_steps*args.gradient_accumulation_steps, dynamic_ncols=True, disable=(rank != 0)) as pbar:
        for step in pbar:
            ...
            if (step>0 and args.sample_step > 0 and (step+1)/args.gradient_accumulation_steps % args.sample_step == 0) or step == args.total_steps * args.gradient_accumulation_steps - 1 or step==0 :
